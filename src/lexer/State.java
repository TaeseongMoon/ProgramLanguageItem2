	...
	START {
		...
		public TransitionOutput transit(ScanContext context) {
			...
			switch ( ch.type() ) {
				...
				case SPECIAL_CHAR:                     //special charactor가 들어온 경
					if (v=='-'||v=='+') {             //부호인경우에 sign상태로 이동
						context.append(v);           //context에 계속 append해주어야함
						return GOTO_SIGN;
					}
					else if (v=='#') {                 //#이 나오면 sharp상태로 이동
						context.append(v);
						return GOTO_SHARP;
					}
					else {                             //그 외에는 type을 알아내서 알맞은 상태로 이동
						context.append(v);
						String lexme = context.getLexime(); 
						return GOTO_MATCHED(TokenType.fromSpecialCharactor(v), lexme);  //TokenType의 special charcator type return mothod를 이용
					}
				case WS:
					return GOTO_START;
				case END_OF_STREAM:
